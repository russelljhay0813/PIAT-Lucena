...
const facultyProfiles = [
  {
    ...
    middleName: "Anita",
    lastName: "Santos",
    email: "maria.santos@bwest.edu.ph",
  ...
  },
  {
    ...
    middleName: "Cruz",
    lastName: "Dela Vega",
    email: "leila.delavega@bwest.edu.ph",
    password: "Password123!",
    program: "Diploma in Hospitality Services and Technology",
  ...
  },
  {
    ...
    middleName: "Bautista",
    lastName: "Reyes",
    email: "roselyn.reyes@bwest.edu.ph",
    password: "Password123!",
    program: "Diploma in Hospitality Services and Technology",
  ...
  },
  {
    ...
    middleName: "Miguel",
    lastName: "Dela Cruz",
    email: "juan.delacruz@bwest.edu.ph",
    password: "Password123!",
    program: "Diploma in Tourism and Travel Services",
    position: "Instructor I",
  },
  {
    firstName: "Patricia",
    middleName: "Lorenzo",
    lastName: "Mercado",
    email: "patricia.mercado@bwest.edu.ph",
    password: "Password123!",
    program: "Diploma in Tourism and Travel Services",
    position: "Assistant Professor",
  },
  {
    ...
    middleName: "Antonio",
    lastName: "Lim",
    email: "ricardo.lim@bwest.edu.ph",
    password: "Password123!",
    program: "Diploma in Tourism and Travel Services",
  ...
  },
  {
    ...
    middleName: "Rose",
    lastName: "Villanueva",
    email: "ana.villanueva@bwest.edu.ph",
    password: "Password123!",
    program: "Diploma in Multimedia Arts and Design",
  ...
  },
  {
    ...
    middleName: "David",
    lastName: "Reyes",
    email: "christian.reyes@bwest.edu.ph",
    password: "Password123!",
    program: "Diploma in Multimedia Arts and Design",
    position: "Assistant Professor",
  },
  {
    ...
    middleName: "Santos",
    lastName: "Navarro",
    email: "bianca.navarro@bwest.edu.ph",
    password: "Password123!",
    program: "Diploma in Multimedia Arts and Design",
    position: "Senior Instructor",
  },
  {
    ...
    middleName: "Pablo",
    lastName: "Ramos",
    email: "eduardo.ramos@bwest.edu.ph",
    password: "Password123!",
  ...
  },
  {
    ...
    middleName: "Joseph",
    lastName: "Palisoc",
    email: "mark.palisoc@bwest.edu.ph",
    password: "Password123!",
    program: "Diploma in Industrial Education (Major in Hotel and Restaurant Services)",
  ...
  },
  {
    ...
    middleName: "Corazon",
    lastName: "Cordova",
    email: "nina.cordova@bwest.edu.ph",
    password: "Password123!",
  ...
  },
  {
    ...
    middleName: "Enrique",
    lastName: "Garcia",
    email: "paolo.garcia@bwest.edu.ph",
    password: "Password123!",
    program: "Diploma in Industrial Education (Major in Multimedia Arts and Design)",
    position: "Assistant Professor",
  },
  {
    ...
    middleName: "Mina",
    lastName: "Mendoza",
    email: "sofia.mendoza@bwest.edu.ph",
    password: "Password123!",
    program: "Diploma in Industrial Education (Major in Multimedia Arts and Design)",
    position: "Senior Instructor",
  },
...
(async function main() {
  try {
    ...
    for (let index = 0; index < facultyProfiles.length; index += 1) {
      ...
      const userCode = `FAC-${String(index + 10001)
        .toString()
        .padStart(5, "0")}`;
      const employeeCode = `EMP-${String(index + 1)
        .toString()
        .padStart(4, "0")}`;
      const passwordHash = hashPassword(profile.password);
      const username =
        `${profile.firstName.toLowerCase()}${profile.lastName.toLowerCase()}`.replace(
          /[^a-z0-9]+/g,
          "",
        );
      const existingUser = await getQuery(
        "SELECT id FROM users WHERE email = ? AND role = ? LIMIT 1",
        [profile.email, "faculty"],
      );
      ...
      const existingFacultyRow = await getQuery("SELECT id FROM faculty WHERE userId = ?", [
        userId,
      ]);
      ...
      const eligibleSubjects = await allQuery(
        `SELECT id, code, title FROM subjects WHERE program = ? AND academicYear = ? AND semester = ? ORDER BY code`,
      ...
      if (chosenSubjects.length === 0) {
        const fallbackSubjects = await allQuery(
          "SELECT id, code, title FROM subjects WHERE program = ? ORDER BY code LIMIT 4",
          [profile.program],
        );
      ...
      for (let subjectIndex = 0; subjectIndex < chosenSubjects.length; subjectIndex += 1) {
        ...
        await runQuery("UPDATE subjects SET facultyId = ?, instructor = ? WHERE id = ?", [
          userId,
          fullName,
          subject.id,
        ]);
        const offeringExists = await getQuery(
          "SELECT id FROM subject_offerings WHERE subjectId = ? AND facultyId = ? AND academicYearId = ? AND semesterId = ?",
          [subject.id, userId, academicYearId, semesterId],
        );
        if (!offeringExists) {
          await runQuery(
            ...
            [
              ...
              null,
              userId,
              makeSchedule(subjectIndex, index),
              makeRoom(subjectIndex, index),
              30,
              "active",
              createdAt,
            ],
    ...
    console.log(
      JSON.stringify(
        {
          facultyCount: await getQuery(
            "SELECT COUNT(*) AS count FROM users WHERE role = 'faculty'",
          ),
          assignedSubjects: await getQuery(
            "SELECT COUNT(*) AS count FROM subjects WHERE facultyId IS NOT NULL AND facultyId != ''",
          ),
