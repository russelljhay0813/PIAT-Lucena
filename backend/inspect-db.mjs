  ...
  db.get("SELECT COUNT(*) AS c FROM curriculum", (err, row) => {
    ...
    db.get("SELECT COUNT(*) AS c FROM subjects", (err2, row2) => {
      ...
      db.get("SELECT COUNT(*) AS c FROM subjects WHERE academicYear IS NULL", (err3, row3) => {
        ...
        db.get(
          "SELECT COUNT(DISTINCT academicYear) AS c FROM subjects WHERE academicYear IS NOT NULL",
          (err4, row4) => {
            if (err4) throw err4;
            console.log("distinctAcademicYears", row4.c);
            db.all(
              "SELECT academicYear, COUNT(*) AS c FROM subjects GROUP BY academicYear ORDER BY academicYear",
              (err5, rows5) => {
                if (err5) throw err5;
                console.log("subjectsByAcademicYear", rows5);
                db.all(
                  "SELECT program, yearLevel, semester, academicYear, COUNT(*) AS c FROM subjects WHERE academicYear IS NOT NULL GROUP BY program, yearLevel, semester, academicYear ORDER BY program, yearLevel, semester, academicYear LIMIT 20",
                  (err6, rows6) => {
                    if (err6) throw err6;
