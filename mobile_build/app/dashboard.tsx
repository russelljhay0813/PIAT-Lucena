...
export default function DashboardScreen() {
  ...
  const { isConnected, lastSyncAt, isSyncing, syncPendingAttendance, pendingCount } = useSyncStore();
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <Card style={styles.card}>
        ...
        <Card.Actions>
          <Button onPress={syncPendingAttendance} disabled={isSyncing}>Sync Now</Button>
        </Card.Actions>
      </Card>
      <Card style={styles.card}>
        <Card.Title title="Today's Assigned Subjects" />
        <Card.Content>
          {subjects.length === 0 ? <Text>No classes found.</Text> : subjects.map((subject) => (
            <List.Item
              key={subject.id}
      ...
      <Button mode="outlined" onPress={async () => { await deleteAuthData(); router.replace("/login"); }}>
        Logout
      </Button>
    </ScrollView>
  );
